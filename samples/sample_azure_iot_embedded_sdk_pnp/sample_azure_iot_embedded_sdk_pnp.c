...
az_json_writer json_builder;
    ...
    if (!az_failed(az_json_writer_init(&json_builder, buff_span, NULL)) &&
        ...
        !az_failed(az_json_writer_append_property_name(&json_builder, reported_max_temp_since_last_reboot)) &&
...
UINT request_id;
    ...
    if (!az_failed(az_json_writer_init(&json_builder, buff_span, NULL)) &&
        !az_failed(az_json_writer_append_begin_object(&json_builder)) &&
        !az_failed(az_json_writer_append_property_name(&json_builder, reported_temp_property_name)) &&
        !az_failed(az_json_writer_append_begin_object(&json_builder)) &&
        !az_failed(az_json_writer_append_property_name(&json_builder, reported_value_property_name)) &&
        !az_failed(az_json_writer_append_double(&json_builder, current_device_temp_value, DOUBLE_DECIMAL_PLACE_DIGITS)) &&
        !az_failed(az_json_writer_append_property_name(&json_builder, reported_status_property_name)) &&
        !az_failed(az_json_writer_append_int32(&json_builder, (int32_t)status)) &&
        !az_failed(az_json_writer_append_property_name(&json_builder, reported_version_property_name)) &&
        !az_failed(az_json_writer_append_int32(&json_builder, (int32_t)version)) &&
        !az_failed(az_json_writer_append_property_name(&json_builder, reported_description_property_name)) &&
        !az_failed(az_json_writer_append_string(&json_builder, description)) &&
        !az_failed(az_json_writer_append_end_object(&json_builder)) &&
        !az_failed(az_json_writer_append_end_object(&json_builder)))
    {
        bytes_copied = (UINT)az_span_size(az_json_writer_get_bytes_used_in_destination(&json_builder));
...
UCHAR time_buf[32];
    ...
    {
        if (!(az_succeeded(az_json_reader_init(&jp, payload_span, NX_NULL)) &&
              az_succeeded(az_json_reader_next_token(&jp)) &&
              az_succeeded(az_json_token_get_string(&jp.token, (CHAR *)time_buf, sizeof(time_buf), (int32_t *)&len))))
        {
             return(NX_NOT_SUCCESSFUL);
        }
    ...
    if (az_succeeded(az_json_writer_init(&json_builder, response, NULL)) &&
        az_succeeded(az_json_writer_append_begin_object(&json_builder)) &&
        az_succeeded(az_json_writer_append_property_name(&json_builder, report_max_temp_name_span)) &&
        az_succeeded(az_json_writer_append_double(&json_builder, device_max_temp, DOUBLE_DECIMAL_PLACE_DIGITS)) &&
        az_succeeded(az_json_writer_append_property_name(&json_builder, report_min_temp_name_span)) &&
        az_succeeded(az_json_writer_append_double(&json_builder, device_min_temp, DOUBLE_DECIMAL_PLACE_DIGITS)) &&
        az_succeeded(az_json_writer_append_property_name(&json_builder, report_avg_temp_name_span)) &&
        az_succeeded(az_json_writer_append_double(&json_builder, device_avg_temp, DOUBLE_DECIMAL_PLACE_DIGITS)) &&
        az_succeeded(az_json_writer_append_property_name(&json_builder, report_start_time_name_span)) &&
        az_succeeded(az_json_writer_append_string(&json_builder, start_time_span)) &&
        az_succeeded(az_json_writer_append_property_name(&json_builder, report_end_time_name_span)) &&
        az_succeeded(az_json_writer_append_string(&json_builder, fake_end_report_time)) &&
...
UINT buffer_length;
    ...
    if(!(az_succeeded(az_json_writer_init(&json_builder, AZ_SPAN_FROM_BUFFER(scratch_buffer), NULL)) &&
         az_succeeded(az_json_writer_append_begin_object(&json_builder)) &&
         az_succeeded(az_json_writer_append_property_name(&json_builder, telemetry_name)) &&
         az_succeeded(az_json_writer_append_double(&json_builder, current_device_temp, DOUBLE_DECIMAL_PLACE_DIGITS)) &&
         az_succeeded(az_json_writer_append_end_object(&json_builder))))
    {
